...
parser.add_argument('--stu_model', type=str, default='models.MSBDNs.Net')
...
parser.add_argument('--teachers', type=str, default=["weights/fogbest.pkl", "weights/nightbest.pkl", "weights/rainbest.pkl", "weights/snowbest.pkl"])
...
def main():
	...
	for checkpoint_path in args.teachers:
		...
		pretrained_model = torch.load(ROOT / checkpoint_path, map_location=lambda storage, loc: storage)
	...
	if args.resume:
		...
		ckt_modules.load_state_dict(checkpoint['ckt_module'])
