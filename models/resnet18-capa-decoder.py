...
#             nn.BatchNorm2d(channel // 8, eps=1e-5, momentum=0.01, affine=True),
#             nn.ReLU(),
...
#
#     def forward(self, x):
#         y1 = torch.max(x, 1)[0].unsqueeze(1)  # 2 1 80 80
#         y2 = torch.mean(x, 1).unsqueeze(1)    # 2 1 80 80
#         y3 = y1 + y2
#         # y = torch.cat((x, y3), dim=1)
#         y = x * y3
#         z = self.pa(y)
#         # t = x * (1 + z)
#         t = x * z
...
class Net(nn.Module):
    def __init__(self, pretrained=False, pretrain_url="", res_blocks=2):
        ...
        self.palayer_1 = PALayer(32)
        self.palayer_0 = PALayer(16)
