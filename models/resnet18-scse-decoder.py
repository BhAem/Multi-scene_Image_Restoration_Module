...
class sSE(nn.Module):
    ...
    def forward(self, U):
        q = self.Conv1x1(U)  # 2 128 80 80 -> 2 1 80 80
        q = self.norm(q)     # 2 1 80 80
        return U * q  # 广播机制
...
class cSE(nn.Module):
    ...
    def forward(self, U):  # 2 128 80 80
        z = self.avgpool(U)  # 2 128 1 1
        z = self.Conv_Squeeze(z)  # 2 64 1 1
        z = self.Conv_Excitation(z)  # 2 128 1 1
        z = self.norm(z)
        return U * z.expand_as(U) # 2 128 80 80
...
class Net(nn.Module):
    def __init__(self, pretrained=False, pretrain_url="", res_blocks=2):
        ...
        self.calayer_3 = scSE(128)
        self.palayer_3 = scSE(128)
        self.calayer_2 = scSE(64)
        self.palayer_2 = scSE(64)
        self.calayer_1 = scSE(32)
        self.palayer_1 = scSE(32)
