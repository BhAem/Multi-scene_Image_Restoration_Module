...
parser.add_argument('--stu_model', type=str, default='models.DesnowNet.DesnowNet')
...
parser.add_argument('--beta', type=int, default=4, help='the scale of the pyramid maxout')
parser.add_argument('--gamma', type=int, default=4, help='the levels of the dilation pyramid')
parser.add_argument('--weight_decay', type=float, default=5e-4)
parser.add_argument('--weight_mask', type=float, default=3, help='the weighting to leverage the importance of snow mask')
parser.add_argument('--mode', type=str, default='original', help='the architectural mode of DesnowNet')
...
def evaluate(model, val_loader, epoch):
    ...
    for target, image in pBar:
        ...
        y_hat, y_, z_hat, za = model(image)

        psnr_list.append(torchPSNR(y_hat, target).item())
        ssim_list.append(pytorch_ssim.ssim(y_hat, target).item())
...
def train_ke_stage(model, train_loader, optimizer, scheduler, epoch):
    ...
    for target_images, input_images in pBar:
        ...
        y_hat, y_, z_hat, za = model(input_images)
        loss1 = lw_pyramid_loss(y_hat, target_images)
        loss2 = lw_pyramid_loss(y_, target_images)
        loss = loss1 + loss2
...
def main():
    ...
    print(Back.RED + 'Using Model: {}'.format(args.stu_model) + Style.RESET_ALL)
    ...
    # Prepare the Model
    model = net_func_student(beta=args.beta, gamma=args.gamma, mode=args.mode).cuda()
    ...
    optimizer = torch.optim.Adam([{'params': model.parameters()}],
                             lr=linear_scaled_lr, weight_decay=args.weight_decay, betas=(0.9, 0.999), eps=1e-8)

    if args.resume:
        optimizer.load_state_dict(checkpoint['optimizer'])

    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=10000, verbose=True)
