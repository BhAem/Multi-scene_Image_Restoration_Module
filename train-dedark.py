...
parser.add_argument('--stu_model', type=str, default='models.ZeroDCE.enhance_net_nopool')
...
def train_ke_stage(model, train_loader, optimizer, scheduler, epoch, L_color, L_spa, L_exp, L_TV):
    start = time.time()
    ...
    for target_images, input_images in pBar:
        ...
        enhanced_image_1, enhanced_image, A = model(input_images)
        Loss_TV = 200 * L_TV(A)
        loss_spa = torch.mean(L_spa(enhanced_image, input_images))
        ...
        meters = update_meter(meters, [total_loss.item(), total_loss.item(), total_loss.item()])
...
def main():
    ...
    L_color = DECloss.L_color()
    L_spa = DECloss.L_spa()
    L_exp = DECloss.L_exp(16, 0.6)
    L_TV = DECloss.L_TV()
    ...
    for epoch in range(start_epoch, args.max_epoch + 1):
        train_ke_stage(model, train_loader, optimizer, scheduler, epoch, L_color, L_spa, L_exp, L_TV)
        # validating
