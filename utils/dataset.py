...
class DatasetForInference2(Dataset):
	...
	def __getitem__(self, index):
		target_path, input_path = self.image_paths[index]
		# huawei
		input_path = '/cache/dataset2/' + input_path
		input_image = Image.open(input_path).convert('RGB')
		input_image = self.transform(input_image)

		# input_image = cv2.imread(input_path)
		# input_image = cv2.cvtColor(input_image, cv2.COLOR_BGR2RGB)
		# h0, w0 = input_image.shape[:2]  # orig hw
		# r = 640 / max(h0, w0)  # ratio
		# if r != 1:  # if sizes are not equal
		# 	input_image = cv2.resize(input_image, (int(w0 * r), int(h0 * r)),
...
class DatasetForInference(Dataset):
	...
	def __getitem__(self, index):
		...
		return input_image, os.path.basename(input_path), (t2-t1)
...
class DatasetForTrain(Dataset):
	...
	def __getitem__(self, index):
		...
		# huawei
		# target_path = '/cache/dataset2/' + target_path
		# input_path = '/cache/dataset2/' + input_path
		...
		# 	input_image = input_image[:, y:y + 64, x:x + 64]
		# 	target_image = target_image[:, y:y + 64, x:x + 64]
